...
class SRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            lr_img = self.transform(lr_img).to(device)
            hr_img = Image.open(hr_path).convert('RGB')
            hr_img = self.transform(hr_img).to(device)
...
def main():
    ...
    srset = SRDataset(transform=my_transform,lr_path=args.lr_dir,hr_path=args.hr_dir)
